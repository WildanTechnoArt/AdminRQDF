...
class FirestoreTeacherAdapter(options: FirestoreRecyclerOptions<TeacherModel>) :
    FirestoreRecyclerAdapter<TeacherModel, FirestoreTeacherAdapter.ViewHolder>(options) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, item: TeacherModel) {
        ...
        holder.apply {
            ...
            containerView.btn_select.setOnClickListener {
                SharedPrefManager.getInstance(context).saveTeacherUID(getUserId)
